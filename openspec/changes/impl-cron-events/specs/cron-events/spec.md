# cron-events Delta Specification
...
交互式提醒结果 MAY 触发本地桌面通知（基线为 macOS `osascript`，且仅限 chat job 与 web channel 的 partner job），为 best-effort：通知失败 SHALL 静默处理，SHALL NOT 影响正常投递。（有意差异：Go 版允许省略该增强或以跨平台通知库实现；不作为验收必需项。）
