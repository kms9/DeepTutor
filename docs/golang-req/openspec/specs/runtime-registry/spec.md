...
## Purpose
本模块定义 Go 版运行时内核的注册与路由层：ToolRegistry 负责工具（Level 1）的注册、注销、别名解析、OpenAI function-calling schema 生成、prompt hint 组装与执行分发；CapabilityRegistry 负责 capability（Level 2）的内置清单加载与查询；ChatOrchestrator 是所有入口（CLI / WebSocket / SDK）的统一路由，将一次 `UnifiedContext` 交给选定 capability，管理该轮的 StreamBus 生命周期并发布完成事件。本 spec 以 Python 基线 v1.5.2 的行为为对等目标。

- 参考实现（基线）：`deeptutor/runtime/orchestrator.py`、`deeptutor/runtime/registry/tool_registry.py`、`deeptutor/runtime/registry/capability_registry.py`、`deeptutor/runtime/bootstrap/builtin_capabilities.py`、`deeptutor/core/tool_protocol.py`、`deeptutor/core/capability_protocol.py`、`deeptutor/tools/builtin/__init__.py`（`TOOL_ALIASES` 等常量）
- 依赖 spec / 里程碑：依赖 foundation-config、foundation-stream；里程碑：M1
