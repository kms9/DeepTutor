# Sidecar Contract Specification

## Purpose
本模块定义 Go 主进程与 Python Sidecar 之间的 typed API 契约。RAG（索引/检索）、文档解析（KB 入库解析与附件轻量抽取两档）、Manim 渲染、Python 代码沙箱执行保留在 Python Sidecar 内，Go 侧以强类型客户端调用。契约的核心约束是：全部请求/响应经 schema 校验、拒绝未知字段、禁止 `dict[str,Any]` 直传；Sidecar 不可用时 chat 主链路不受影响，仅依赖 Sidecar 的功能返回明确错误。

- 参考实现（基线）：`deeptutor/services/rag/service.py`、`deeptutor/services/rag/factory.py`、`deeptutor/services/parsing/`（`service.py`、`types.py`）、`deeptutor/services/sandbox/`（`service.py`、`spec.py`）、`deeptutor/agents/math_animator/renderer.py`、`deeptutor/utils/document_extractor.py`、`deeptutor/api/routers/knowledge.py`（progress WS）
- 依赖 spec / 里程碑：依赖：foundation-config（PathService/settings）、foundation-stream（进度事件转发）；里程碑：M3
