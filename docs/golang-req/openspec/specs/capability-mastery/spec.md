# capability-mastery Specification

## Purpose
本模块定义 Go 版 `mastery_path` capability（Guided Learning）与其底层 learning 引擎的目标行为。设计公理与基线一致：智能在 loop 出口（模型决定教什么、怎么提问），而"学习者能否前进"的门（gate）是确定性引擎计算——按 knowledge point 的 topic type 门控：MEMORY / PROCEDURE 走定量门（掌握度 ≥ 0.9），CONCEPT / DESIGN 走定性门（Feynman 式讲解由 tutor 判定）。capability 本身复用 chat agent loop，仅追加五个 `mastery_*` 工具与 tutor system prompt；grading / mastery / scheduler 三个确定性算法 SHALL 保证 Go 版对相同输入产出与 Python 完全一致。存储为每 path 一个 JSON 文件；`/api/v1/learning` REST 提供 dashboard 与路径管理。

- 参考实现（基线）：`deeptutor/capabilities/mastery/capability.py`、`deeptutor/capabilities/mastery/loop.py`、`deeptutor/capabilities/mastery/tools.py`、`deeptutor/capabilities/mastery/choices.py`、`deeptutor/capabilities/mastery/prompts/{en,zh}/system.md`、`deeptutor/learning/{grading.py, mastery.py, scheduler.py, policy.py, service.py, storage.py, models.py, prompts.py}`、`deeptutor/api/routers/mastery_path.py`
- 依赖 spec / 里程碑：依赖 capability-chat-solve（chat loop）、runtime-registry、turn-runtime、session-store；里程碑：M3
