# turn-runtime Specification

## Purpose
本模块定义 Turn 级运行时：接收 start_turn 载荷后在后台运行一次完整用户轮，组装 `UnifiedContext`（历史压缩、附件解析、KB/Notebook/Book/Persona/Skills/Memory、模型与工具白名单、多用户权限），驱动 orchestrator，统一分配事件 seq、持久化事件与 assistant 消息，并对外提供 `subscribe_turn` 回放、cancel、regenerate/消息分支、`ask_user` 暂停/恢复等能力。本 spec 以 Python 基线 v1.5.2 行为为对等目标，并显式标注两处有意差异：`waiting_input` 状态与 append-before-publish 事件持久化。

- 参考实现（基线）：`deeptutor/services/session/turn_runtime.py`、`deeptutor/services/session/context_builder.py`、`deeptutor/services/session/source_inventory.py`、`deeptutor/core/context.py`、`deeptutor/services/session/sqlite_store.py`（turns / turn_events 语义）
- 依赖 spec / 里程碑：依赖 foundation-stream、session-store、runtime-registry、agent-loop、sidecar-contract（附件解析）；里程碑：M1
