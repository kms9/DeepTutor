# api-unified-ws Specification

## Purpose
本模块定义统一 WebSocket 端点 `/api/v1/ws` 的协议面：全部 11 种客户端消息类型（`message`/`start_turn`、`ping`、`subscribe_turn`、`subscribe_session`、`check_active_turn`、`resume_from`、`unsubscribe`、`cancel_turn`、`submit_user_reply`、`regenerate`、`user_input`）的请求/响应/事件推送语义、错误处理、心跳约定与连接/订阅生命周期。现有 `web/` 前端零改动复用，故本 spec 的消息形状与事件字段以基线为字节级对等目标。

- 参考实现（基线）：`deeptutor/api/routers/unified_ws.py`、`deeptutor/services/session/turn_runtime.py`（subscribe/cancel/reply 语义）、`deeptutor/core/stream_bus.py`（bus 注册表与 `user_input`）、`web/lib/unified-ws.ts`（前端心跳与重连预期）
- 依赖 spec / 里程碑：依赖 turn-runtime、foundation-stream、multi-user-auth（WS 鉴权门）；里程碑：M1
