# Foundation Stream Specification

## Purpose
本模块定义 deeptutor-go 的统一流式事件协议 `StreamEvent` 与单 turn 内的异步扇出总线 `StreamBus`。所有 tool、capability 与消费端（CLI 渲染、WebSocket 推送、SDK）都以同一事件信封通信；前端逐字段依赖该信封，Go 版序列化结果必须与 Python 基线逐字段对等（字段级完整对等由 contracts/ 下的 golden fixtures 保证）。

- 参考实现（基线）：`deeptutor/core/stream.py`、`deeptutor/core/stream_bus.py`、`deeptutor/services/session/turn_runtime.py`（`_publish_live_event` 的 seq 分配）
