...
## Purpose
本模块定义 deeptutor-go 的 Partner（IM 伴侣）子系统：每个 partner 是一个由 chat agent loop 驱动的常驻实例，拥有独立 workspace（SOUL.md、memory、skills、KB、notebook）与工具策略；来自 IM channel 或 Web 的每条消息都作为一个 chat turn 进入 `ChatOrchestrator` 的同一执行链。模块包含 partner manager（生命周期与配置持久化）、channel adapter 抽象与 outbound 分发管线、首批四个 channel（feishu/telegram/slack/discord），以及 `/api/v1/partners` REST 面（约 31 条）与 `/api/v1/partners/{id}/ws` Web 聊天通道。磁盘布局与基线 `data/partners/<id>/` 完全兼容，Go 版可直接打开既有数据。

- 参考实现（基线）：`deeptutor/services/partners/`（`manager.py`、`runtime.py`、`workspace.py`、`sessions.py`、`commands.py`、`scope.py`）、`deeptutor/partners/channels/`（`base.py`、`manager.py`、`registry.py`、`feishu.py`、`telegram.py`、`slack.py`、`discord.py`）、`deeptutor/partners/bus/`、`deeptutor/partners/config/`、`deeptutor/api/routers/partners.py`
- 依赖 spec / 里程碑：依赖：capability-chat-solve、turn-runtime、agent-loop、multi-user-auth（partner 合成 scope 与 admin 门控）；里程碑：M4
