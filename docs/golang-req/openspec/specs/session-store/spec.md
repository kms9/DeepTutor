...
## Purpose
本模块定义 deeptutor-go 的会话持久层与 sessions REST API：以现有 SQLite 库 `data/user/chat_history.db` 为唯一事实源，schema 与 Python 基线兼容——由 Python v1.5.2 创建的老库 SHALL 免迁移直接打开。覆盖 sessions/messages（含 `parent_message_id` 编辑分支）/turns/turn_events（事件回放）/notebook_entries/notebook_categories 六类表、store API 语义以及 `/api/v1/sessions` 各路由的对外契约。

- 参考实现（基线）：`deeptutor/services/session/sqlite_store.py`、`deeptutor/services/session/protocol.py`、`deeptutor/services/session/turn_runtime.py`、`deeptutor/api/routers/sessions.py`
- 依赖 spec / 里程碑：依赖：foundation-config（PathService）、foundation-stream（事件信封）；里程碑：M1
