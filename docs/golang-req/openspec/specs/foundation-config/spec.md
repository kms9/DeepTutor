# Foundation Config Specification

## Purpose
本模块定义 deeptutor-go 的运行时配置层与路径服务：以 `data/user/settings/*.json` 为唯一持久化配置源，进程环境变量作为显式部署级 override，项目根目录的 `.env` 文件被有意忽略。`PathService` 集中管理 `data/` 目录布局，是所有磁盘写入（KB、memory、workspace、SQLite、日志）的唯一路径来源。Go 版必须能直接打开现有 Python 版生成的 `data/` 目录与 settings 文件，免迁移复用。

- 参考实现（基线）：`deeptutor/services/config/runtime_settings.py`、`deeptutor/services/path_service.py`、`deeptutor/services/file_io.py`（atomic_write_json）、`data/user/settings/` 下实际文件清单
- 依赖 spec / 里程碑：依赖：无；里程碑：M0
