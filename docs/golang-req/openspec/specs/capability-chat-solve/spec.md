# capability-chat-solve Specification

## Purpose
本模块定义 Go 版 `chat` 与 `deep_solve` 两个 capability 的目标行为。`chat` 是默认 capability（M1 交付）：一次用户 turn 运行单一 agentic loop（每轮一次 LLM 调用 + 工具分发），文本实时流式给用户，模型自行决定何时结束。`deep_solve`（M3 交付）不再有独立 pipeline：它复用同一个 chat loop，仅在 turn 上打 `solve_mode` 标记、注入 solve 专用 system prompt，并挂载 `solve_plan` / `solve_finish_step` / `solve_replan` 三件套工具，由内存态 `SolveSession` 提供确定性脊柱（计划承诺、逐步完成门、限额 replan）。本 spec 以 Python 基线 v1.5.2 行为为对等目标。

- 参考实现（基线）：`deeptutor/agents/chat/capability.py`、`deeptutor/agents/chat/agentic_pipeline.py`、`deeptutor/agents/chat/agent_loop.py`、`deeptutor/agents/chat/prompt_blocks.py`、`deeptutor/agents/chat/prompts/{en,zh}/agentic_chat.yaml`、`deeptutor/capabilities/solve/capability.py`、`deeptutor/capabilities/solve/loop.py`、`deeptutor/capabilities/solve/tools.py`、`deeptutor/capabilities/solve/session.py`、`deeptutor/capabilities/solve/prompts/{en,zh}/system.md`、`deeptutor/agents/_shared/capability_result.py`
- 依赖 spec / 里程碑：依赖 runtime-registry、agent-loop、turn-runtime、llm-provider、tools-builtin；里程碑：M3（chat 部分 M1）
