# skills-persona Specification

## Purpose

本模块定义技能（SKILL.md 能力包，模型按需取用）与 persona（PERSONA.md 语气/行为预设，eager 注入 system prompt）两套用户可编辑资产在 Go 侧的目标行为：SKILL.md 解析与 manifest 渲染、builtin/user 双层存储、`read_skill` / `load_tools` 工具的服务侧语义、skills REST（`/api/v1/skills`）、hub 导入安全门，以及 persona 的注入语义与 personas REST（`/api/v1/personas`）。partner 的 `SOUL.md` 是 partners 模块的 persona 对应物，不在本 spec 范围（仅要求 persona 注入接口可被复用）。

- 参考实现（基线）：`deeptutor/services/skill/`（`service.py`、`hub.py`、`taxonomy.py`）、`deeptutor/services/persona/service.py`、`deeptutor/skills/builtin/`、`deeptutor/api/routers/skills.py`、`personas.py`
- 依赖 spec / 里程碑：依赖 foundation-config（workspace 路径）；被 agent-loop（manifest / always / persona 注入）与 tools-builtin（read_skill/load_tools）依赖；里程碑：M2
