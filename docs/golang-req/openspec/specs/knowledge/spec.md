...
本模块定义知识库（KB）子系统在 Go 侧的目标行为：KB 生命周期（创建/删除/raw 文件管理/索引版本 `version-N`/连接型 KB）、embedding 兼容性 reconcile、初始化/重建进度追踪与 `/progress/ws` 推送、knowledge REST（`/api/v1/knowledge`，约 46 条路由，按功能组约束），以及 RAG provider 配置面。索引与检索的 pipeline 本体（llamaindex/pageindex/graphrag/lightrag 的实现）不在本 spec——Go 侧只做 facade：管理目录/注册表/进度，把 index/query 请求转发给 Python Sidecar（见 sidecar-contract spec）。数据布局 `data/knowledge_bases/<name>/` 必须与基线兼容，老 KB 免迁移可用。

- 参考实现（基线）：`deeptutor/knowledge/`（`manager.py`、`initializer.py`、`add_documents.py`、`kb_types.py`、`naming.py`、`progress_tracker.py`）、`deeptutor/api/routers/knowledge.py`、`deeptutor/multi_user/knowledge_access.py`
- 依赖 spec / 里程碑：依赖 foundation-config、sidecar-contract（索引/检索/解析转发）、multi-user-auth（KB 归属与可见性，M2 阶段可先以单用户路径实现）；里程碑：M2
