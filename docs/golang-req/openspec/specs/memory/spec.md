...
本模块定义三层文件型记忆子系统在 Go 侧的目标行为：L1 append-only 事件 trace、L2 per-surface 摘要文档、L3 跨 surface 综合文档，以及 LLM 驱动的 consolidator（update / audit / dedup / merge 四操作）、memory workbench REST（`/api/v1/memory`，约 27 条路由）与 `read_memory` / `write_memory` 工具的存储侧语义。全部数据为纯文件（markdown + JSONL + meta JSON），必须与基线 v1.5.2 的老 `memory/` 目录逐字节兼容（免迁移续写）。consolidator 的 LLM 调用经 llm-provider；工具面契约见 tools-builtin spec。

- 参考实现（基线）：`deeptutor/services/memory/`（`store.py`、`trace.py`、`paths.py`、`ids.py`、`ops.py`、`document.py`、`settings.py`、`consolidator/`、`snapshot/`）、`deeptutor/api/routers/memory.py`
- 依赖 spec / 里程碑：依赖 foundation-config（路径服务）、llm-provider（consolidator LLM 调用）；被 tools-builtin（read_memory/write_memory）与 capability 各 surface 的 trace 挂钩依赖；里程碑：M2
